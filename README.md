...
- **Forwarding throughput:** at least **10 Gbps per node** sustained encrypted forwarding baseline (bidirectional aggregate) on commodity hardware (**8+ physical cores, 32 GB RAM, 25 GbE NIC with kernel-bypass support**). This is a minimum admission threshold for production rollout; higher line-rate utilization remains a stretch target.
...
- Hybrid key exchange: **x25519 + ML-KEM-768**, combined via HKDF over concatenated shared secrets with domain separation (`x25519_shared_secret || mlkem768_shared_secret`) for interoperable key schedule derivation
