...
func NewHybridSession(combinedSecret, sessionInfo []byte) (*HybridSession, error) {
	...
	// Random nonce base — unique per session.
	if _, err := rand.Read(s.nonceBase[:]); err != nil {
		return nil, fmt.Errorf("crypto: nonce base generation: %w", err)
	}

	// Extra seq mask from key material (derive after the key so they're independent).
	var mask [8]byte
